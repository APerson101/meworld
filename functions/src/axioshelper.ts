...
export class axiosHelper {
  ...
  async getgiftCrd2() {
    const url = "https://api-testbed.giftbit.com/papi/v1/brands?currencyisocode=USD&embeddable=true";

    let result;
    await axios.get(url, {headers: this.APIKEY}).then((res) => {
      console.log(res.data); result = res.data;
    }).then((burst) => console.log(burst));
    return result;
  }
    ...
    const res = axios.put(request["url"], request["body"], {headers: request["headers"], params: request["params"]}).then(
      ...
      (val) => {
        if (axios.isAxiosError(val)) {
          console.log(val); return val.response?.data;
        }
      }
    ); return res;
  }
